...
From the 7 models tested on the test set, it appears that MA(2) was most successful with the lowest RMSE at about 3332 and MAE at about 2569. This is the same as ARIMA(0,1,2) model. We can now apply this model to our validation set to see how successful our model is in its prediction.
...
We see that the predictive power is not very good. If we check  how we did in terms of RMSE and MAE:

```{r}
accuracy(best_model_fc, gold_validation_df)
```

We see that our typical error is now about 4,130 INR in terms of RMSE, and 3,690 INR in terms of MAE. Interestingly if we extend the MA(2) forecast success we got from the smaller training set to a longer forecast projection of 879 days that combines the test and validation sets we get better accuracy:
...
If we check the accuracy:

```{r}
accuracy(ma2_extended_fc, gold_validation_df)
```

We see that RMSE is now as low as 1558 and MAE is as low as 1292. The predictive accuracy of the most successful data is due to two lines of thinking. First, differencing closing day prices from the immediately preceding day's closing price ensured we have stationarity. Stationarity, in turn, allowed us to examine how using past forecast errors in linear-regression fashion, which gave us our best model. When trained on the bigger training set, however, the forecasting accuracy of the model dropped. Interestingly, we could see that if we attempt to predict from the smaller training set, our accuracy improved considerably.
...
This work attempted to identify the best model for predicting gold mini futures closing prices. It did so by dividing the data into training set and validation set at 90-10% split, first, then further dividing the training set into smaller training set and a test-set at 80-20% split. This meant the smaller training set had 72% of the original data, while test set had 18%, and validation set 10%. The success wa defined a the smallest RMSE and MAE, interpreted as typical error in predicting gold mini futures closing price. In total, 7 different models were trained on the smaller training set in Section 2 and tested on the test set. The most successful among those were then validated on the validation set in Section 3.

The work, however, suffers from a number of limitations as not every line of inquiry has been pursued. The work has not explored, and recommends for future research the exploration of dynamic regression models, stochastic trends, and lagged predictors. Equally, it is important to examine the percentage change behaviour and trading volume in these models.

Finally, the data and the references can be found in the references section, below. The code and other models are on github.com/emre-us.
...
Dataset: https://www.kaggle.com/datasets/nisargchodavadiya/daily-gold-price-20152021-time-series?select=Gold+Price.csv
