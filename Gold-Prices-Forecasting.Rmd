...
We will begin by splitting our data to training and test, or holdout, sets. The latter will be held out to test the most successful model. The most successful model will be determined by cross-validating the training data whereby a series of test sets are generated and tested on. The corresponding training sets for each test set consists of gold mini futures closing price observations that occurred prior to those observation(s) in the test set.
...
We can also check the dimensions of the training and test sets:

```{r}
tibble(training_set = dim(gold_train_df),
      test_set = dim(gold_test_df))
```
and we see that training set has 2825 rows and test set 314 rows. Both has 7 columns as before.
